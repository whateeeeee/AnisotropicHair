...
CUSTOM_NAMESPACE_START(BxDF)
    ...
    half3 StandardBRDF(CustomLitData customLitData,CustomSurfacedata customSurfaceData,half3 L,half3 lightColor,float shadow)
    {
        ...
        float stepNoL = step(0.5,NoL);
        diffuseTerm = lerp(diffuseTerm * 0.1,diffuseTerm,stepNoL);

        float stepNoL2 = step(0.65,NoL);
        diffuseTerm = lerp(diffuseTerm * 0.5,diffuseTerm,stepNoL2);
        //TODO npr specular
        float stepNoV = step(0.98,NoV);
        diffuseTerm = lerp(diffuseTerm ,diffuseTerm * 1.6,stepNoV) * lightColor;
        ...
        float3 specularTerm = SpecularGGX(a2,customSurfaceData.specular,NoH,NoV,NoL,VoH);;
		    ...
		    specularTerm = half3(0,0,0);
	    #endif
        //return customSurfaceData.IML.bbbb;
        //return  (diffuseTerm + specularTerm) * radiance;
        half3 finalLighting = diffuseTerm + specularTerm * radiance;
        return  finalLighting * customSurfaceData.IML.g;
    }
    ...
    half3 EnvBRDF(CustomLitData customLitData,CustomSurfacedata customSurfaceData,float envRotation,float3 positionWS)
    {
       ...
       // R = half3(0,0,0);
       // R = BoxProjectedCubemapDirection(R, customLitData.positionWS, unity_SpecCube0_ProbePosition, unity_SpecCube0_BoxMin, unity_SpecCube0_BoxMax);
	    ...
	    #endif
        return (indirectDiffuseTerm + indirectSpecularTerm )* saturate(customSurfaceData.IML.gggg + 0.3);
    }
    ...
    half3 ComplexEnvBRDF(CustomLitData customLitData,CustomSurfacedata customSurfaceData,CustomClearCoatData customClearCoatData,float envRotation,float3 positionWS)
    {
        ...
        //ClearCoat
        NoV = saturate(abs(dot(customClearCoatData.clearCoatNormal,customLitData.V)) + 1e-5);//区分正反面
        R = reflect(-customLitData.V,customClearCoatData.clearCoatNormal);
